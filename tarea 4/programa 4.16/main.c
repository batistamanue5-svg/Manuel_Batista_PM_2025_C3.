...
/* Temperaturas.
El programa recibe como datos 24 numeros reales que representan las temperaturas en el exterior en un periodo de 24 horas. Calcula el promedio del dia y las temperaturas maxima y minima con la hora e la que se registraron. */
...
int HMAX;
int HMIN;
/* Variables globales. ACT se utiliza para acumular las temperaturas, por esa razon se inicializa en cero. MAX se utiliza para calcular la maxima; se inicializa en -50 para que el primer valor que se ingrese modifique su contenido. MIN se usa para calcular la minima; se inicializa con un valor muy alto para que el primer valor ingresado modifique su contenido. HMAX y HMIN se utilizan para almacenar el horario en que produjeron las temperaturas maxima y minima, respectivamente. */
...
/* Esta funcion almacena la temperatura minima y la hora en que se produjo en la variables globales MIN y HMIN. */
{
if (MIN > T)
{
    MIN = T;
